...
export function brokenStrongCombos(hand: readonly Tile[], discard: Tile): ReadonlyArray<readonly [number, number]> {
  ...
  const remainingValues = new Set(
    hand.filter(tile => sameSuit(tile) && !(tile.value === discard.value)).map(tile => tile.value),
  )
...
export function countOpportunities(hand: readonly Tile[], visible: readonly Tile[] = [], options: OpportunityOptions = {}): OpportunityResult {
  ...
  for (const type of types) {
    for (let value = 1; value <= 9; value++) {
      ...
      if (isWinningHand(trial, { dingque: options.dingque ?? null }))
        waits.push({ tile: candidate, remaining })
    }
  }
  return { total: waits.reduce((sum, wait) => sum + wait.remaining, 0), waits }
}
...
/**
 * 根据听牌进张形态分类。14 张听牌手牌，统计有效进张的分布：
 * - 进张种类 1 且同花色相邻值差 2 → kanchan
 * - 进张种类 1 且相邻 → single/twoSided 视具体结构
 * - 进张种类 ≥ 3 → threeSided 以上
 * 简化为按进张种类数与连续性估算，供启发式使用。
 */
export function classifyWaitShape(waits: OpportunityResult['waits']): WaitShape {
  if (waits.length === 0)
    return 'other'
  if (waits.length >= 3)
    return 'threeSided'
  if (waits.length === 1) {
    const value = waits[0].tile.value
    if (value === 1 || value === 9)
      return 'single'
    return 'kanchan'
  }
...
/** 防守出牌安全分（0-1，越高越安全）：熟张/跟打 > 尾盘边张 > 踩线安全线 > 危险线 */
export function safetyScore(value: number, isShouZhang: boolean, isLateGame: boolean): number {
  if (isShouZhang)
    return 1
  if (isLateGame && isTerminal(value))
    return 0.8
  // 踩线避炮：1/4/7 线间隔大、成搭少，相对安全；2/5/8 次之；3/6/9 最易被吃碰
...
/** 判断某次杠是否破坏牌型结构：杠牌会固定 3 张，若导致剩余手牌无法正常组搭，视为破坏 */
export function gangBreaksStructure(handCountAfterGang: number, meldsAfterGang: number): boolean {
  // 血战到底：胡牌需 4 面子 + 1 将；杠算一组面子。
  // 杠后若剩余手牌不足以支撑剩余面子+将，则破坏结构。
  const neededMelds = 4 - meldsAfterGang
  const expectedHandCount = neededMelds * 3 + 2
